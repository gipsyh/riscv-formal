...
def write_invariants_file(project_dir):
    with open(f"{project_dir}/invariants.sv", "w") as f:
        print("module invariants (", file=f)
        print("    input logic clock,", file=f)
        print("    input logic reset", file=f)
        print(");", file=f)
        print("", file=f)
        ...
        print("", file=f)
        print("bind rvfi_testbench invariants invariants (.*);", file=f)
...
def write_ric3_project(check, source_files, include_files, include_dirs, generated):
    ...
    project_dir = f"{cfgname}/{check}"
    write_invariants_file(project_dir)
